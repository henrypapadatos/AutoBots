...
def train(epoch, config, train_loader, net, loss, post_process, opt, val_loader=None):
    ...
    display_iters = int(
        config["display_iters"] / (1 * config["batch_size"])
    )
    val_iters = int(config["val_iters"] / (1 * config["batch_size"]))

    start_time = time.time()
    metrics = dict()
    for i, data in tqdm(enumerate(train_loader),disable=1):
...
def val(config, data_loader, net, loss, post_process, epoch):
    ...
    for i, data in enumerate(data_loader):
        data = dict(data)
        with torch.no_grad():
            output, node_embeddings = net(data, epoch=0, train=False)
            loss_out = loss(output, node_embeddings, data, train=False)
            post_out = post_process(output, data)
            post_process.append(metrics, loss_out, post_out)
