...
class Distance2Intersection:
    ...
    def reset(self, graph, selected_nodes, dis_matrix):
        self.node_idcs = graph['idcs']
        self.selected_node = selected_nodes
    ...
    def transform_data(self, epoch):
        ...
        for i in range(len(self.node_idcs)):
            ...
            selected_node = np.array(self.selected_node[i]) + start_idx
            dis_matrix = np.array(self.dis_matrix[i])

            sub_index = np.where(dis_matrix < 1000)
            dis_matrix = dis_matrix[sub_index]
            selected_node = selected_node[sub_index]

            dis_matrix[dis_matrix > 5] = 1000 
            dis_matrix = 1/dis_matrix + 1e-10

            self.selected_nodes.append(selected_node)
            self.label.append(dis_matrix)

            start_idx += len(self.node_idcs[i])

        self.selected_nodes = list(np.concatenate(self.selected_nodes))
        self.pseudo_labels_feats = torch.tensor(list(np.concatenate(self.label))).to(self.device)

    def make_loss(self, embeddings):
        embeddings_feats = self.linear1(embeddings[self.selected_nodes])
        num_total = len(self.selected_nodes)
        loss = F.mse_loss(embeddings_feats.squeeze(1).float(), self.pseudo_labels_feats.float(), reduction='sum') / (num_total + 1e-10)
        return loss
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pred_net = PredNet(config)
         # --------------new!
        self.ssl_agent = Distance2Intersection(nhid=config['n_map'], device='cuda')
        # --------------new!
    ...
    def forward(self, data: Dict, epoch, train=True) -> Dict[str, List[Tensor]]:
        ...
        if train:
            ...
            dis_matrix = data['dis_matrix']

        # --------------new!
            self.ssl_agent.reset(graph, selected_nodes, dis_matrix)
...
def graph_gather(graphs):
    ...
    #graph["suc_pairs"] = [x["suc_pairs"] for x in graphs]
    #graph["left_pairs"] = [x["left_pairs"] for x in graphs]
    #graph["right_pairs"] = [x["right_pairs"] for x in graphs]
    # --------new!--------------------
