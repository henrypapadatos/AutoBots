...
class AttributeMask():
    ...
    def transform_data(self, mask_ratio=0.2):
        ...
        for i in range(len(self.node_idcs)):
            ...
            dist = self.cached_actors.view(-1, 1, 2) - self.cached_ctrs[nnodes].view(1, -1, 2)
            dist = torch.sqrt((dist ** 2).sum(2))
            mask_dist = torch.where(dist <= 15.0)

            nnodes_th = torch.unique(mask_dist[1]).cpu().numpy().copy()
            nnodes_bg = np.delete(nnodes, nnodes_th)
            perm1 = np.random.permutation(nnodes_th)
            perm2 = np.random.permutation(nnodes_bg)

            masked_node = np.concatenate([perm1[: int(len(perm1) * mask_ratio)], perm2[: int(len(perm2) * mask_ratio)]])
    ...
    def make_loss(self, embeddings):
        ...
        loss = 0.7 * F.mse_loss(masked_embeddings_feats, self.pseudo_labels_feats, reduction='mean') + \
               0.3 * F.mse_loss(masked_embeddings_ctrs, self.pseudo_labels_ctrs, reduction='mean')
        return loss
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        #--------------new!
        self.ssl_agent = AttributeMask(nhid=config['n_map'], device='cuda')
        # --------------new!

    def forward(self, data: Dict, train=True) -> Dict[str, List[Tensor]]:
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, node_embeddings, data: Dict, train=True, ssl_agent=None) -> Dict:
        ...
        loss_out["loss"] = loss_out["cls_loss"] / (loss_out["num_cls"] + 1e-10 ) + loss_out["reg_loss"] / (loss_out["num_reg"] + 1e-10) + loss_ssl
