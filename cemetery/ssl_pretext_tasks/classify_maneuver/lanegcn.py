...
class IntentionTask:
    ...
    def reset(self, intentions, actor_idcs):
        self.intention_cls = intentions
        self.actor_idcs = actor_idcs

    def transform_data(self, epoch):
        self.selected_ids = []
        for i in range(len(self.actor_idcs)):
            self.selected_ids.append(self.actor_idcs[i][0].cpu().numpy())
        self.selected_ids = np.concatenate([self.selected_ids], 0)
        self.label = self.intention_cls
        self.pseudo_labels_feats = torch.tensor(self.label).type(torch.LongTensor) .to(self.device)
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        # --------------new!
        self.ssl_agent = IntentionTask(nhid=config['n_map'], device='cuda')
        # --------------new!
    ...
    def forward(self, data: Dict, epoch, train=True) -> Dict[str, List[Tensor]]:
        ...
        node_embeddings = actors.clone()

        if train:
            # --------------new!
            intentions = gpu([x for x in data['intention_cls']])
            self.ssl_agent.reset(intentions, actor_idcs)
