...
class SuccessFailure:
    def __init__(self, config, device='cuda'):
        self.device = device
        self.a2m = A2M(config).to(device)
        self.num_classes = 2
        self.nhid = config['n_map']
        self.linear = nn.Linear(self.nhid, self.num_classes).to(device)
        self.loss = FocalLoss()

    def reset(self, graph, feats, endpoints, actor_ctrs, actor_idcs):
        self.ctrs = torch.cat(graph['ctrs'], 0).clone().to(self.device)
        self.feats = feats.clone().to(self.device)
        self.endpoints = endpoints
        self.node_idcs = graph["idcs"]
        self.actor_idcs = actor_idcs
        self.actor_ctrs = actor_ctrs

        self.graph_new = dict()
        self.graph_new['turn'] = graph['turn'].clone()
        self.graph_new['control'] = graph['control'].clone()
        self.graph_new['intersect'] = graph['intersect'].clone()

    def transform_data(self, epoch):
        targets = []
        graph_idcs = []
        graph_ctrs = []
        ...
        for i in range(len(self.node_idcs)):
            ...
            dist_pos = self.endpoints[i].view(-1, 1, 2) - self.ctrs[nnodes].view(1, -1, 2)
            dist = torch.sqrt((dist_pos ** 2).sum(2))
            pos_idcs = torch.where(dist[0] <= 2.0)[0].cpu().numpy()

            dist_neg = self.actor_ctrs[i][0].view(-1, 1, 2) - self.ctrs[nnodes].view(1, -1, 2)
            dist = torch.sqrt((dist_neg ** 2).sum(2))
            neg_idcs = torch.where(dist[0] <= 30.0)[0].cpu().numpy()
            neg_idcs = np.array([x for x in neg_idcs if x not in pos_idcs])

            perm1 = np.random.permutation(pos_idcs)
            labels1 = np.ones_like(perm1)
            perm2 = np.random.permutation(neg_idcs)  # [:int(neg_idcs.shape[0]*0.4)]
            labels2 = np.zeros_like(perm2)
            batch_selected_idcs = np.concatenate([perm1, perm2])
            batch_labels = np.concatenate([labels1, labels2])
            perm = np.random.permutation(batch_selected_idcs.shape[0])
            batch_selected_idcs = batch_selected_idcs[perm]
            batch_labels = batch_labels[perm]
            graph_idcs.append(batch_selected_idcs)
            targets.append(batch_labels)
            graph_ctrs.append(self.ctrs[batch_selected_idcs])

        self.graph_new['feats'] = self.feats[np.concatenate(graph_idcs)]
        self.graph_new['turn'] = self.graph_new['turn'][np.concatenate(graph_idcs)]
        self.graph_new['control'] = self.graph_new['control'][np.concatenate(graph_idcs)]
        self.graph_new['intersect'] = self.graph_new['intersect'][np.concatenate(graph_idcs)]
        self.graph_new['idcs'] = [torch.tensor(x).to(self.device) for x in graph_idcs]
        self.graph_new['ctrs'] = graph_ctrs
        self.targets = targets

    def make_loss(self, embeddings):
        targets = torch.cat([torch.tensor(x).to(self.device) for x in self.targets], 0)
        graph_nodes_feat = self.a2m(self.graph_new['feats'], self.graph_new, embeddings, self.actor_idcs, self.actor_ctrs)
        preds = self.linear(graph_nodes_feat)
        preds = torch.softmax(preds, dim=1)
        range_n = torch.arange(0, preds.shape[0], dtype=torch.int64, device=self.device)
        return self.loss(preds[range_n, targets], targets)
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        # --------------new!
        self.ssl_agent = SuccessFailure(config, device='cuda')
        # --------------new!

    def forward(self, data: Dict, epoch, train=True) -> Dict[str, List[Tensor]]:
        ...
        if train:
            endpoints = gpu([x for x in data['endpoint']])
        ...
        embeddings = actors.clone()

        if train:
            # --------------new!
            self.ssl_agent.reset(graph, nodes, endpoints, actor_ctrs, actor_idcs)
            self.ssl_agent.transform_data(epoch)
            # --------------new!
        # prediction
...
class A2M(nn.Module):
    ...
    def forward(self, feat: Tensor,
                ...
                actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor]) -> Tensor:
...
class Loss(nn.Module):
    ...
    def forward(self, out: Dict, node_embeddings, data: Dict, train=True, ssl_agent=None) -> Dict:
        loss_ssl = 0.0
        if train:
            loss_ssl = ssl_agent.make_loss(node_embeddings)
        loss_out = self.pred_loss(out, gpu(data["gt_preds"]), gpu(data["has_preds"]))
        
        loss_out["loss"] = loss_out["cls_loss"] / (loss_out["num_cls"] + 1e-10 ) + loss_out["reg_loss"] / (loss_out["num_reg"] + 1e-10) + loss_out["goal_loss"] / (loss_out["num_goal"] + 1e-10) + loss_ssl
